...
export function strictJsonParse(text) {
  if (typeof text !== 'string') {
    fail('json-text-required', 'registry parsing requires exactly one primitive JSON text input');
  }
  ...
  function stringValue(maxCodeUnits, sizeCode, sizeLabel) {
    ...
    while (cursor < text.length) {
      ...
      if (!escaped && code === 0x22) {
        ...
        if (result.length > maxCodeUnits) fail(sizeCode, `${sizeLabel} exceeds the supported size`);
        return result;
      }
  ...
  function value(depth) {
    ...
    if (!Number.isSafeInteger(number) || Object.is(number, -0)) {
      fail('json-number', 'JSON numbers must be safe integers and not negative zero');
    }
    return number;
  }
