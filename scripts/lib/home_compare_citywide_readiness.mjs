...
export function validateHomeCompareCitywideReadiness(value) {
  ...
  for (const [i, source] of value.sources.entries()) { exact(source,['source_id','ordinal','receipt_identity','status','freshness','clocks','coverage','dq','official_source_url','limitations'],`source ${i}`); if (source.source_id!==SOURCE_IDS[i] || source.ordinal!==i || !SHA.test(source.receipt_identity||'') || !['available','available-zero','partial','unavailable'].includes(source.status) || !/^https:\/\//.test(source.official_source_url) || !Array.isArray(source.limitations) || !source.limitations.length || !source.freshness?.status || !source.clocks || !source.coverage?.status || !source.dq?.status) throw new Error('Citywide readiness source is invalid.'); }
  for (const [i, dimension] of value.dimensions.entries()) { if (dimension.dimension!==DIMENSIONS[i] || dimension.ordinal!==i || dimension.required_source_receipt_identities?.[0]!==value.sources[i].receipt_identity || !SHA.test(dimension.identity||'') || dimension.total!==null || dimension.available_zero!==false || dimension.row_availability!=='unavailable' || dimension.value_availability!=='unavailable') throw new Error('Citywide readiness dimension must remain aggregate-only and fail closed.'); }
