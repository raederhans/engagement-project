const MAX_JSON_CODE_UNITS = 32_000_000;
const MAX_DEPTH = 64;
const MAX_ITEMS = 2_000_000;
const BLOCKED_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

export const REAL_COMPACT_JSON_LIMITS = Object.freeze({
  maxCodeUnits: MAX_JSON_CODE_UNITS,
  maxDepth: MAX_DEPTH,
  maxItems: MAX_ITEMS,
});

function fail(message) {
  throw new TypeError(`RealCompactGraph JSON contract: ${message}`);
}

/**
 * Parses one bounded primitive JSON string without consulting caller-owned
 * objects. Duplicate keys, blocked prototype keys, unsafe numbers, excessive
 * depth, and excessive aggregate item counts are rejected before admission.
 */
export function strictRealCompactJsonParse(text) {
  if (typeof text !== 'string') {
    fail('input must be primitive JSON text; object, Proxy, getter, descriptor, symbol, and sparse inputs are forbidden');
  }
  ...
  function stringValue() {
    ...
    while (cursor < text.length) {
      ...
      if (!escaped && code < 0x20) {
        fail('JSON strings must not contain raw control characters');
      }
