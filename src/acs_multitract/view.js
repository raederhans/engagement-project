const COPY = Object.freeze({
  en: Object.freeze({
    ...
    intro: 'Choose at least 2 complete Philadelphia tracts, then review and calculate their combined population.',
    browse: 'Browse available tracts', choose: 'Available tract', addTract: 'Add tract',
    catalogLoading: 'Loading available tracts…', catalogFailed: 'Tract list unavailable. Retry or enter GEOIDs below.',
    selected: 'Selected tracts (editable GEOIDs)',
    quickCalculate: 'Review & calculate', searchTracts: 'Find a GEOID',
    manual: 'Review step by step (optional)',
    export: 'Download result and source notes (JSON)',
    exportFailed: 'Download could not start. Your result is still available; try again.',
  ...
  zh: Object.freeze({
    eyebrow: '完整人口普查区比较',
    title: '比较完整人口普查区',
    intro: '选择至少 2 个完整费城普查区，核对后计算合计人口。',
    browse: '浏览可用普查区', choose: '可用普查区', addTract: '添加普查区',
    catalogLoading: '正在加载可用普查区…', catalogFailed: '普查区列表暂不可用，请重试或在下方输入 GEOID。',
    selected: '已选普查区（可编辑 GEOID）',
    quickCalculate: '核对并计算', searchTracts: '搜索 GEOID',
    manual: '分步核对（可选）',
    export: '下载结果与来源说明（JSON）',
    exportFailed: '下载未能启动，计算结果仍保留，请重试。',
...
export function acsMultitractProductHtml(locale = 'en') {
  ...
  return [
    ...
    `<button class="button button--secondary" type="button" data-acs-browse>${escapeHtml(copy.browse)}</button>`,
    '<div class="acs-multitract__picker" data-acs-picker hidden>',
    `<label for="acs-tract-search">${escapeHtml(copy.searchTracts)}</label><input id="acs-tract-search" class="field" type="search" data-acs-search>`,
    `<label for="acs-tract-choice">${escapeHtml(copy.choose)}</label>`,
    '<select id="acs-tract-choice" class="field" data-acs-choice></select>',
    `<button class="button button--secondary" type="button" data-acs-add>${escapeHtml(copy.addTract)}</button>`,
    '</div>',
    '<p data-acs-catalog-status role="status" hidden></p>',
    `<label for="acs-multitract-input">${escapeHtml(copy.selected)}</label>`,
    ...
    `<button class="button button--primary" type="button" data-acs-quick>${escapeHtml(copy.quickCalculate)}</button>`,
    '</div>',
    `<details class="workspace-disclosure" data-acs-manual><summary>${escapeHtml(copy.manual)}</summary><div class="acs-multitract__actions">`,
    ...
    `<button class="button button--secondary" type="button" data-acs-export hidden>${escapeHtml(copy.export)}</button>`,
    '</div>',
