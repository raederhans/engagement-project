...
function getStatusMessages(locale) {
  if (locale === 'zh-CN') {
    return {
      ...
      'address-duplicate': '多个住宅对应同一个标准化地址，本次比较已停止。',
      'address-geography-conflict': '候选地址指向不同位置，本次比较已停止。',
      'address-unavailable': '当前版本暂不支持地址比较。输入内容未发送到外部服务。',
      'parcel-missing': '未找到对应地块，无法继续比较。',
      'parcel-ambiguous': '地址对应多个地块，请核对地址。',
      'parcel-duplicate': '多个地址对应同一地块，请更换比较对象。',
      'parcel-address-mismatch': '地址服务与 OPA 房产地址不一致，本次比较已停止。',
      'parcel-geography-mismatch': '地址服务与 OPA 房产位置不一致，本次比较已停止。',
      'source-unavailable': '所需数据暂不可用，无法生成比较结果。',
      'results-unavailable': '结果暂时无法显示，请重试。',
  ...
  return {
    ...
    'address-duplicate': 'Multiple homes resolve to the same standardized address, so the comparison stopped.',
    'address-geography-conflict': 'The address candidates point to different locations, so the comparison stopped.',
    'address-unavailable': 'Private-address comparison is disabled in this public build. The process stopped before contacting an address, parcel, map, or related service.',
    'parcel-missing': 'No matching property parcel was found.',
    'parcel-ambiguous': 'The address matches multiple parcels, so the comparison stopped.',
    'parcel-duplicate': 'These addresses refer to the same property. Choose different addresses.',
    'parcel-address-mismatch': 'The City address service and OPA property address disagree, so the comparison stopped.',
    'parcel-geography-mismatch': 'The City address service and OPA property location disagree, so the comparison stopped.',
    'source-unavailable': 'Required data is unavailable. Comparison results cannot be generated.',
