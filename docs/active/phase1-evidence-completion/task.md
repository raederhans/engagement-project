...
| Execution evidence record | pending this record commit | The new raw `external-observed.json` is byte-bound to `6e714789`; this record-only commit anchors it without self-writing its own SHA. |
| Cumulative evidence record | pending following record-only commit | The following record-only candidate will bind implementation `6e714789` and this execution record; its own identity is resolved by `git rev-parse HEAD`, never self-written. `cumulativeTip` and `reviewedTip` remain null. |
