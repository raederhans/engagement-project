...
`orchestrating` — S2 scope, dependency gates, and lane ownership are frozen on a
new coordination branch; four isolated tasks are about to be created from the
same exact baseline.
...
- [ ] Commit the S2 orchestration baseline.
- [ ] Create and start the S2-0 Contract / Product Semantics task.
- [ ] Create and start the S2-1 Search Algorithm task in read-only preparation.
- [ ] Create and start the S2-2 Observation / Data Admission task in read-only
  preparation.
- [ ] Create and start the S2-3 Golden / Independent Verification task in
  read-only preparation.
- [ ] Register task IDs, worktrees, exact bases, and initial states.
- [ ] Send narrow semantic follow-ups to the three original research tasks.
...
- The four task IDs/worktree paths do not exist until task creation completes and
  must be written back before implementation handoff.
