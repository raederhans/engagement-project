...
## S1.1 — Architecture hardening & stress run (2025-11-11 15:50)
- Commands:
  - `git switch -c feat/diary-stab-s1`
  - `npm ci` (fails: `esbuild.exe` locked) → `taskkill /IM esbuild.exe /F && taskkill /IM node.exe /F`
  - `rm -rf node_modules && npm install`
  - `npm run dev` (manual 20-cycle stress on route select → alt toggle → play/pause/finish loop)
- MapLibre surface check via `window.__diary_debug`:
...
After 20 rapid cycles:
  Sources: ["diary-segments","diary-route-overlay","diary-alt-route","diary-sim-point"]
  Layers:  ["diary-segments-line","diary-route-overlay-line","diary-alt-route-line","diary-sim-point-circle"]
```

- Heap snapshot (Chrome devtools) stored at `logs/screenshots/s1_stress_heap.png` — confirms resident set stabilized after teardown.
- Files touched: `src/map/segments_layer.js`, `src/map/routing_overlay.js`, `src/routes_diary/index.js`
  - Added `ensureSource`/`ensureLineLayer`/`ensureCircleLayer` helpers so add/remove operations stay idempotent under rapid toggles
  - Centralized transient teardown via `teardownDiaryTransient()` + `stopAllTimersAndListeners()`
  - Expanded `window.__diary_debug` clone-only exports for quick auditing of sources/layers/sim state

## S1.2 — Deterministic demo data + validator (2025-11-11 15:58)
- Commands:
  - `npm run data:gen`
  - `npm run data:check`
- Output:
...
- Deliverables: `scripts/generate_demo_data.mjs`, `scripts/validate_demo_data.mjs`, refreshed `data/*.demo.geojson`, and `docs/TEST_DATA_README.md` (operator playbook).

## S1.3 — Math + aggregator verification scripts (2025-11-11 16:05)
- Commands:
...
```
[Diary Tests] PASS — decay math helpers stable.
[Diary Tests] PASS — aggregator updates behave deterministically.
```

- Artifacts: `scripts/tests/diary_math.mjs`, `scripts/tests/aggregator_update.mjs`; npm aliases wired through `package.json`.
